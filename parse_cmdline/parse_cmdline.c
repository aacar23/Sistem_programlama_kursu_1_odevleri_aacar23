...
static void free_dynamic_pointer_array(struct dynamic_pointer_array *arr)
{
    free(arr -> pointers);
    free(arr);
}
...
static struct dynamic_string *create_dynamic_string(const char *default_string)
{
    ...
    if (!new)
        return NULL;
    new -> core_string = calloc((new -> capacity = 2 * (new -> size = len)) + 1, sizeof(char));
...
char **parse_cmdline(const char *cmdline)
{
    ...
    free_dynamic_pointer_array(arr);
    return ret;
}
